...
from .base import ModelCapability, ProviderAdapter, ProviderError, PROVIDER_OLLAMA_LOCAL
...
class OllamaLocalProvider(ProviderAdapter):
    """Provider adapter for local Ollama model discovery."""

    name = PROVIDER_OLLAMA_LOCAL
    fallback_model_ids = FALLBACK_MODELS

    def list_models(self, config: AppConfig) -> list[str]:
        endpoint = f"{_normalize_base_url(config.ollama_local_base_url)}/api/tags"
    ...
    def analyze_transcript(
        ...
        endpoint = f"{_normalize_base_url(config.ollama_local_base_url)}{OLLAMA_CHAT_PATH}"
        payload = {
